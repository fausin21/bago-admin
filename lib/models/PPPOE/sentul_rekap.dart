...
class RekapSentul extends StatefulWidget {
  @override
  _RekapSentulState createState() => _RekapSentulState();
}

class _RekapSentulState extends State<RekapSentul>
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://aplikasi.bago.web.id/api/bago/bago_baru/pppoe_lunas.php?id=2'),
      );
            ...
            children: [
              ...
              if (searchQuery.trim().isNotEmpty)
                TextButton.icon(
                  ...
                  icon:
                      Icon(Icons.refresh, size: 16, color: Colors.purple[600]),
                  label: Text(
                    'Reset Filter',
                    style: TextStyle(
          ...
          if (searchQuery.trim().isNotEmpty) ...[
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  Icon(Icons.search, size: 14, color: Colors.purple[600]),
                  ...
                  GestureDetector(
                    ...
                    child:
                        Icon(Icons.close, size: 14, color: Colors.purple[600]),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildPaymentCard(dynamic rekap) {
    ...
    Color statusColor = status == 'LS'
        ? Colors.green
        : status == '3'
            ? Colors.blue
            : Colors.orange;
    String statusText = status == 'LS'
        ? 'Lunas'
        : status == '3'
            ? 'Transfer'
            : 'Pending';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              final q = await showSearch<String?>(
                context: context,
                delegate: RekapSearchDelegate(rekapData),
              );
          ...
          IconButton(
            icon: Icon(Icons.save, color: Colors.purple[600]),
            onPressed: saveData,
            tooltip: 'Simpan Data',
          ),
          IconButton(
...
class RekapSearchDelegate extends SearchDelegate<String?> {
  final List<dynamic> data;
  List<String> recent = [];

  RekapSearchDelegate(this.data);
