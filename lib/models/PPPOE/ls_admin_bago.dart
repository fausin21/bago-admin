...
class LsAdminBago extends StatefulWidget {
  @override
  _LsAdminBagoState createState() => _LsAdminBagoState();
}

class _LsAdminBagoState extends State<LsAdminBago>
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://aplikasi.bago.web.id/api/admin/admin_baru/pppoe_ls_admin.php?id=1'),
      );
  ...
  Future<void> getSharedPrefData() async {
    ...
    setState(() {
      cabang = prefs.getString('nama') ?? 'BAGO';
    });
  }
              ...
              child: Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child:
                        Icon(Icons.close, size: 14, color: Colors.orange[700]),
                  ),
                ],
              ),
            ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.search, color: Colors.orange[700]),
            onPressed: () async {
              final q = await showSearch<String?>(
                context: context,
                delegate: LsAdminBagoSearchDelegate(rekapData),
              );
          ...
          IconButton(
            icon: Icon(Icons.save, color: Colors.orange[700]),
            onPressed: saveData,
            tooltip: 'Simpan Data',
          ),
          IconButton(
...
class LsAdminBagoSearchDelegate extends SearchDelegate<String?> {
  final List<dynamic> data;
  List<String> recent = [];

  LsAdminBagoSearchDelegate(this.data);
